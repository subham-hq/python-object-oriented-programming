...
Work top to bottom; later sections lean on earlier ones. Sections 08–10 are the
intermediate/advanced tier — do them after the core (01–07) is solid.

1. `01_foundations/` — classes, methods, class vs instance variables, class/static methods
1. `02_inheritance/` — single & multiple inheritance, MRO, mixins, isinstance vs issubclass
1. `03_dunder_methods/` — repr/str, operators, container/iterator/context-manager protocols, eq/hash
1. `04_encapsulation_and_properties/` — name mangling, properties, descriptors
1. `05_abstraction_and_typing/` — abstract base classes, protocols & duck typing
1. `06_composition/` — composition vs inheritance
1. `07_modern_python/` — dataclasses, slots, enums
1. `08_advanced_dunders/` — callable objects, dynamic attribute access
1. `09_applied_oop/` — custom exceptions, cached_property, advanced dataclasses
1. `10_object_model/` — `__new__` vs `__init__`, `__init_subclass__`, copy semantics
...
|11|arithmetic_and_comparison      |Operators incl. reflected (`__rmul__`); `total_ordering`; `NotImplemented`.|
|12|container_protocol             |`__len__/__getitem__/__setitem__/__contains__` → behaves like a container. |
|13|iterator_protocol              |`__iter__/__next__` + the generator (`yield`) form of `__iter__`.          |
...
|30|new_vs_init                    |`__new__` creates the instance, `__init__` initialises it.                 |
|31|init_subclass                  |`__init_subclass__` auto-registers/validates subclasses (no metaclass).    |
|32|copy_and_deepcopy              |Shallow copy shares nested objects; deep copy duplicates them.             |
